...
// Basic opcodes
var opcodes = ["", "SET", "ADD", "SUB", "MUL", "DIV", "MOD", "SHL", "SHR",
...
function hex(v) { return "0x{0}".fmt(flexo.pad(v.toString(16), 4)); }
...
function decode(word)
{
  ...
  if (op) {
    ...
    return [opcodes[op], a, b];
  } else {
    // Extended opcode
    var o = (word & 0x03f0) >> 4;
    var a = (word & 0xfc00) >> 10;
    return [ext_opcodes[o], a];
  }
}
...
{
  if (v < 0x08) return abc[v];
  if (v < 0x10) return "[{0}]".fmt(abc[v - 8]);
  if (v < 0x18) return "[0x{0} + {1}]".fmt(w.toString(16), abc[v - 16]);
...
var machine =
{
  ...
  set_register: function(i, v)
  {
    ...
    flexo.notify(this, "@register", { which: abc[i], value: this.registers[i] });
  ...
  exec: function()
  {
    ...
    flexo.log("{0}: {1}".fmt(hex(this.pc), disassemble(w, w2, w3)));
    this.set_pc(pc);
    if (!this.skip) {
