var svg_draw =
{
  ...
  init: function(div, error)
  {
    ...
    this.svg.setAttribute("viewBox", "0 0 {0} {1}".fmt(div.offsetWidth,
          div.offsetHeight))
    this.svg.appendChild(flexo.svg("g", { display: "none" }));  // for redo
  ...
  handleEvent: function(e)
  {
    ...
    if (e.type === "mousedown" && e.button === 0) {
      ...
      var stroke = STROKE
      this.__path = flexo.svg("path", { stroke: STROKE.value, fill: FILL.value,
        "stroke-width": parseFloat(WIDTH.value) } );
  ...
  undo: function()
  {
    var last = this.svg.lastElementChild;
    if (last && last !== this.svg.firstChild) {
      this.svg.removeChild(last);
      this.redo_.push(last);
    }
  },
  ...
  redo: function()
  {
    var last = this.redo_.pop();
    if (last) this.svg.appendChild(last);
  },
